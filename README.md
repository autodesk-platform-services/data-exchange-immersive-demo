...
- [`DataExchangeConversionService/`](DataExchangeConversionService/) — ASP.NET (.NET 10) web service that extracts geometry from [Data Exchanges](https://aps.autodesk.com/data-exchange-cover-page) using the generally available Data Exchange .NET SDK v8 and converts it into OBJ/MTL, GLB (glTF binary), and USDZ artifacts. Exposes a small REST API (under `/api/jobs`) to start a conversion, poll its status, download the resulting artifacts, and delete them.
